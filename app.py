...
def welcome():
    ...
    return (
        ...
        f"Type in a single date (i.e., 2016-01-01) to see the min, max and avg temperature since that date.<br/>"
...
def tobs():
    ...
    tobs_previous = session.query(Measurement.station, Measurement.date, Measurement.tobs). \
    ...
    temp_data = []
    for tobs_data in tobs_previous:
        tobs_data_dict = {}
...
def start(start_date, end_date):
    ...
    minimum = session.query(Measurement.tobs, func.min(Measurement.tobs)).filter(
        Measurement.date.between(start_date, end_date))
    maximum = session.query(Measurement.tobs, func.max(Measurement.tobs)).filter(
        Measurement.date.between(start_date, end_date))
    average = session.query(Measurement.tobs, func.avg(Measurement.tobs)).filter(
        Measurement.date.between(start_date, end_date))
